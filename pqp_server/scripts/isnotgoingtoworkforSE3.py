...
def is_collision(sx, sy, sz, gx, gy, gz, rr, okdtree):
    ...
    for i in range(nstep):
        idxs, dist = okdtree.search(np.matrix([x, y, z]).T)
        if dist[0] <= rr:
        ...
        z += D* math.cos(yaw)
    # goal point check
    idxs, dist = okdtree.search(np.matrix([gx, gy, gz]).T)
    if dist[0] <= rr:
        return True  # collision
    ...
    if(sx < gx and sy < gy and sz < gz):
        ...
        tempy = sy+yrate
        tempz = sz+zrate
        while(tempx<gx and tempy<gy and temp<gz):
            T=[tempx, tempy, tempz]
            if (pqp_client(T, R)):
		#print "2"
                return True
...
def generate_roadmap(sample_x, sample_y, sample_z, rr, obkdtree):
    ...
    for (i, ix, iy, iz) in zip(range(nsample), sample_x, sample_y, sample_z):
        ...
        index, dists = skdtree.search(
            np.matrix([ix, iy, iz]).T, k=nsample)
        inds = index[0][0] #come back to this
        edge_id = []
        #  print(index)
        ...
        for ii in range(1, len(inds)):
            ...
            ny = sample_y[inds[ii]]
	    nz = sample_z[inds[ii]]
            if not is_collision(ix, iy, iz, nx, ny, ny, rr, obkdtree):
...
def dijkstra_planning(sx, sy, sz, gx, gy, gz, ox, oy, rr, road_map, sample_x, sample_y, sample_z):
    """
...
def sample_points(sx, sy, sz, gx, gy, gz, rr, ox, oy, obkdtree):
    maxx = max(ox)
    maxy = max(oy)
    minx = min(ox)
    miny = min(oy)
	...
	tz = random.uniform(0, 2.7)
        #print (tx, ty)
        index, dist = obkdtree.search(np.matrix([tx, ty, tz]).T)
...
def prm():
    ...
    rx, ry, rz = PRM_planning(sx, sy, sz, gx, gy, gz, ox, oy, robot_size)
    #print rx
    #print ry
    print " done planning"
    path = []
